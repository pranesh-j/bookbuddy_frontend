...
import { getAllBooks, getBook, updateBookTitle, simplifyText } from '@/lib/api';
...
const BookBuddy = () => {
  ...
  const handleSaveTitle = async (title: string) => {
    try {
        ...
        setIsReading(false);  // Return to input view
    } catch (error) {
              ...
              >
                ...
                >
                  <div className="flex items-center space-x-4 overflow-x-auto pb-4 px-4">
                    ...
                    <motion.button
                        onClick={() => {
                            if (pages.length === 1) {  // If we're on the first page
